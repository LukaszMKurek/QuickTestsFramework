...
{
   [TestFixture]
   public sealed class ThrowAssertFailTests
   {
      private ThrowAssertFailTestsInternal _ut;

      [TestFixtureSetUp]
      public void SetUp()
      {
         _ut = new ThrowAssertFailTestsInternal();
...
___ Test case OK _________", typeof(AssertionException), @"Jeden z przypadków testowych nie przeszed³.");
      ...
      {
         AssertInvoke.That(() => _ut.ExceptionInAserrt(), @"*** Test case 1 ***
...
ExceptionBeforeRun; before;", typeof(AssertionException), @"Assert.Fail message");
      }
